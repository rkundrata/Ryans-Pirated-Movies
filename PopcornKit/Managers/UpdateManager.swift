    ...
    private func checkForUpdates(_ completion: ((_ success: Bool) -> Void)? = nil) {
        ...
        Alamofire.request("https://api.github.com/repos/rkundrata/Ryans-Pirated-Movies/releases").validate().responseJSON { (response) in
            ...
            if let latestRelease = sortedReleases.first,
                ...
                let alert = UIAlertController(title: "Update Available".localized, message: .localizedStringWithFormat("%@ version %@ of Ryans Pirated Movies is now available.".localized, latestRelease.releaseType.rawValue.localized, latestRelease.buildNumber), preferredStyle: .alert)
